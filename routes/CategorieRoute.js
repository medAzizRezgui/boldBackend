...
//update categorie
    router.patch('/:catId', async (req,res)=>{
        try{
        const updatedCategorie = await categorie.updateOne({_id : req.params.catId},{$set :{name :req.body.name}});
        res.status(200).send('updated successfully :' + updatedCategorie.acknowledged);
        }catch(err){
          res.status(400).send({message : err});
