...
router.get("/getall", async (req, res) => {
    try {
      const SousCategories = await SousCategorie.find().populate("categorie","name")
  ...
  router.delete('/delete/:SousCategorieId',async (req,res)=>{
    try{
     const removedSousCategorie = await SousCategorie.deleteOne({_id : req.params.SousCategorieId});
    ...
    router.patch('/:souscatId', async (req,res)=>{
      try{
      const updatedSousCategorie = await categorie.updateOne({_id : req.params.souscatId},{$set :{name :req.body.name}});
      res.status(200).send('updated successfully :' + updatedSousCategorie.acknowledged);
      }catch(err){
        res.status(400).send({message : err});
